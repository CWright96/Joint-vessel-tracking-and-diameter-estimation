% This function give mask center and its radius and give back a vector that
% each element show integral in different radius

% Example:
% create_input(origial image, mask center, mask size, previous direction,
% rotation step, mu for guassian, and sigma)

function [output] = create_input(input_image, seed_point_position, vessel_radius,previous_direction)

% seed_point_position = accurate_centerline(end,:);
% previous_direction = direction(end);
zoom = 4;
...
% for i =1:3
    ...
    output((i-1)*179+1:i*179) = integral_179/max(integral_179);
    plot(output)
% end
end